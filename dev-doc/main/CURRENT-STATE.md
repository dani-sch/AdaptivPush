...
- AP-02 and AP-03 are integration-verified at their prior baseline. The 2026-09-11 device-hardening packet is locally verified on Android 16: direct React Navigation resolution, current-only swaps, immutable successor revisions, explicit recalibration, owner-scoped route recovery, cold offline launch, reload, background/foreground, kill/reopen, stale-route handling and an actual account switch all pass against local Supabase.
- AP-02/AP-03 are not released: production remains at the two AP-01 migrations, both writer flags remain default-off, the Free plan provides no managed backup, the CLI needs secure reauthentication, and physical-device, TalkBack/dynamic-type/theme, archive/restore, manual-program and old-client coverage remain open.
...
The approved `integrator` worktree now exists at `C:\workout-app\AdaptivPush-integrator`; clean-install, static, focused and fresh-reset SQL verification passed there. The project-local Supabase CLI remains pinned and Docker supplies PostgreSQL 17 locally. A user-local Android SDK, JDK 17 and Android 16 emulator now support local native development-build checks. The current CLI session still lacks platform authentication. Historical unreferenced avatar objects remain outside scope and were not deleted.
...
Focused gates now include `test:catalog` (9), dependency resolution (1), `test:workouts` (13) and `test:programs` (10). On the feature branch, strict types, lint, Expo doctor 21/21, a 3,819-module Android Metro export, native debug assemble, fresh four-migration reset, database lint, AP-01 regression SQL, AP-02/AP-03 transaction/isolation SQL and successor-revision SQL pass. Current lint has zero errors and three unrelated warnings. Production backup/restore, deployment, physical-device/accessibility/old-client coverage and later fitness/legal/purchase/moderation gates remain open. Keep both writer flags off until release gates pass. Rollback disables producers and preserves drafts, receipts, revisions, checkpoints, accepted artifacts and history; it never restores unsafe catalog writes, non-atomic multiwrites, hidden readiness escalation or false completion.
