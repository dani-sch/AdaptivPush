...
- AP-02 and AP-03 are integration-verified locally. They add versioned workout/program contracts, owner-scoped durable pending state, atomic replay-safe finalization/installation commands, immutable program revisions, one-active-per-owner enforcement, explicit workout outcomes and exact V2 checkpoints. Their deterministic writer flags default off.
- AP-02/AP-03 are not released: production remains at the two AP-01 migrations, the Free plan provides no managed backup, the CLI needs secure reauthentication, and no Expo-capable device/emulator is available for the required reconnect/accessibility matrix.
...
Actual Quick Setup/Profile/Generate Program and AP-02/AP-03 workout/program UI on an Expo-capable simulator/device remain open, including runtime missing-schema, kill/offline/reconnect, background timer, account switch, dynamic type, screen reader, touch targets and theme checks. The attempted historical web fallback is not device evidence. The production ledger contains only the verified AP-01 baseline and catalog enforcement migrations. The Free project still has no scheduled backup/PITR; the prior AP-01 recovery set is not a fresh pre-AP-02/AP-03 recovery point, so a new encrypted logical backup and restore comparison is mandatory before production migration.

The approved `integrator` worktree now exists at `C:\workout-app\AdaptivPush-integrator`; clean-install, static, focused and fresh-reset SQL verification passed there. The project-local Supabase CLI remains pinned and Docker supplies PostgreSQL 17 locally. The current CLI session lacks platform authentication, and no Expo device/emulator or Android SDK is available. Historical unreferenced avatar objects remain outside scope and were not deleted.
...
Focused gates now include `test:catalog` (9), `test:workouts` (5) and `test:programs` (4). Strict types, lint, AP-01 regression SQL, AP-02/AP-03 transaction/isolation SQL, fresh reset, database lint and local push dry-run pass in the integrator. The nine lint warnings are in source files unchanged from the implementation base. Production backup/restore, deployment, runtime/device, fitness/legal/purchase/moderation gates do not close from local integration. Keep both new writer flags off until release gates pass. Rollback disables producers and preserves drafts, receipts, revisions, checkpoints, accepted artifacts and history; it never restores unsafe catalog writes, non-atomic multiwrites, hidden readiness escalation or false completion.
