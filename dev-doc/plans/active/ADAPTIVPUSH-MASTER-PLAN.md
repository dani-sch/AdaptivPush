...
CODE (September 18): `features/catalog/`, `features/kernel/`, `features/programs/`, `features/auth/` and `features/workouts/` implement the first modular boundaries. Generated/manual programs use a validated atomic installer; workouts use owner-scoped frozen drafts, durable pending operations, atomic finalization/correction receipts, removal tombstones, and explicit effective structure. Scoped swaps, removals, additions, and completed corrections are hosted. Auth/session recovery distinguishes connectivity failure from sign-out. `hooks/useCurrentProgram.ts` remains a compatibility facade with legacy progression/date behavior; some screen reads remain direct. The status document separates current code, hosted support, and unverified device behavior.
...
Current execution position: AP-01.3 and the AP-02/AP-03 hosted baseline are deployed, with normal local writer flags enabled. Correction/effective-occurrence migrations `20260915190000` and `20260915210000`, scoped-removal migration `20260917180000`, and structure migration `20260918160000` are also deployed after separate authorization, recovery, rehearsal, and drift/preservation checks. Hosted capabilities are correction 2, removal 1, and structure 1. The current client adds resilient auth/draft recovery and detailed scoped Add/Swap behavior; 157 application cases, strict TypeScript, and lint pass. [September 18 evidence](/dev-doc/reports/ADAPTIVPUSH-WORKOUT-EDITING-2026-09-18.md) owns the latest packet and remaining physical acceptance. Missing required work holds progression; AP-04 dated scheduling and AP-05 authoritative history/progression remain queued.
