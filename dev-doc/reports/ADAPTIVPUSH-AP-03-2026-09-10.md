...
the writer flag defaults off, the migration is not in production, and the
authenticated Expo-device matrix is unavailable.
...
- `utils/saveProgramToDb.ts` resolves the full catalog before mutation, rejects
  any incomplete mapping, builds complete context and installs atomically.
...
| Gate | Result |
|---|---|
| Contract fixtures | `npm run test:programs`: 4 passed: default/custom naming, incomplete mapping rejection, presentation-depth independence and free manual trainability. |
...
| Acceptance | Result |
|---|---|
| AC-TR-016 | Local/integration pass for immutable current revision and stable completed references; regeneration UX remains device-verified only after rollout. |
| AC-TR-017 | Pass for atomic failure, replay, mismatch and stale competing activation. |
| AC-TR-018 | Source path accepts optional demographics/weight; authenticated interruption/device proof remains open. AP-06 still owns richer goal/availability constraints. |
...
| AC-TR-022 | Pass for blank default and custom naming. |
| AC-TR-023 | Pass at contract/source level: free manual build uses common installer; authenticated device proof remains open. |
...
AP-02 artifact. Integration merge commits before evidence closeout are
`c6bc219`, `8884d5f` and `7acc28f`.
