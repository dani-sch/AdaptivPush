...
export function createProgramRepository(supabase: SupabaseClient, enabled = rollout.atomicProgramWriter): ProgramRepository { return {
  async install(input) {
    requireRollout(enabled, 'Atomic program installation');
  ...
  async archive(input) {
    requireRollout(enabled, 'Program archiving');
    const { data, error } = await runSupabaseOperation(
  ...
  async restore(input) {
    requireRollout(enabled, 'Program restoration');
    const { data, error } = await runSupabaseOperation(
