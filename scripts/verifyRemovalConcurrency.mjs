...
try {
  ...
  workout.frozenPrescription = { slots: workout.slots.map(slot => ({ ...slot, sets: slot.sets.map(set => ({ ...set })) })) };
  ...
  const removalRequest = { ...correction, operationId: randomUUID(), expectedRevision: 2, sets: [],
    removals: { version: 1, slots: [], sets: [{ slotId: slots[1], setId: workout.slots[0].sets[0].setId, order: 1 }] },
    programRemoval: { programId: install.programId, expectedRevision: 3, expectedRevisionId: repeated.revisionId,
      currentStableDayId: days[1], targets: [{ slotId: slots[1], order: 1 }] } };
  const removal = await race(`public.correct_workout_removals_v1(${json(removalRequest)})`);
  assert.equal(removal.programRemoval.futureChangedSlotCount, 1);
  assert.equal(await sql(`SELECT current_revision FROM public.programs WHERE id='${install.programId}'`), '4');
  assert.equal(await sql(`SELECT correction_revision FROM public.workout_sessions WHERE id='${finalized.sessionId}'`), '3');
  const removalRace = await Promise.allSettled([0, 1].map(() => rpc(`public.correct_workout_removals_v1(${json({ ...removalRequest, operationId: randomUUID(), expectedRevision: 3, programRemoval: { ...removalRequest.programRemoval, expectedRevision: 4, expectedRevisionId: removal.programRemoval.revisionId } })})`)));
  assert.equal(removalRace.filter(r => r.status === 'fulfilled').length, 1);
  assert.match(removalRace.find(r => r.status === 'rejected').reason.message, /stale_revision/);
  console.log('PASS scoped-removal concurrent exact replay and competing corrections remain atomic');
  const archived = await race(`public.archive_program_v2('${randomUUID()}','${install.programId}',4,'{"week":1}')`);
  ...
  const replacements = await Promise.allSettled([0, 1].map(() => rpc(`public.install_program_v2(${json({ operationId: randomUUID(),
    expectedActiveProgramId: install.programId, expectedActiveRevision: 4, artifact })})`)));
