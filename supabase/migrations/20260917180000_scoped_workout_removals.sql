-- Prepared locally. Hosted application requires separate explicit authorization.
-- Original counts, loads, positions and catalog rows remain immutable.
ALTER TABLE public.program_day_exercises ADD COLUMN removal_mask jsonb;
...
ALTER TABLE public.program_revisions ADD CONSTRAINT program_revisions_provenance_check
  CHECK (provenance IN ('installed', 'migration_snapshot', 'exercise_swap', 'workout_removal'));

-- All revision clone paths (including existing swaps) inherit tombstones.
CREATE FUNCTION public.inherit_prescription_removals() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public AS $$
BEGIN
  IF NEW.removal_mask IS NULL THEN
    SELECT ancestor.removal_mask INTO NEW.removal_mask FROM public.program_day_exercises ancestor
    JOIN public.program_revisions successor ON successor.parent_revision_id = ancestor.program_revision_id
    WHERE successor.id = NEW.program_revision_id AND ancestor.stable_slot_id = NEW.stable_slot_id;
  END IF;
  RETURN NEW;
END $$;
REVOKE ALL ON FUNCTION public.inherit_prescription_removals() FROM PUBLIC, anon, authenticated;
CREATE TRIGGER inherit_prescription_removals BEFORE INSERT ON public.program_day_exercises
FOR EACH ROW EXECUTE FUNCTION public.inherit_prescription_removals();

CREATE FUNCTION public.workout_removal_capability_v1() RETURNS integer
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = pg_catalog AS $$ SELECT 1 $$;

CREATE FUNCTION public.program_removal_state_v1(p_program_id uuid) RETURNS jsonb
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = pg_catalog, public AS $$
DECLARE result jsonb;
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'unauthenticated'; END IF;
  IF NOT EXISTS (SELECT 1 FROM public.programs WHERE id=p_program_id AND user_id=auth.uid()) THEN RAISE EXCEPTION 'forbidden'; END IF;
  SELECT COALESCE(jsonb_object_agg(pde.stable_slot_id::text, pde.removal_mask), '{}'::jsonb) INTO result
  FROM public.program_day_exercises pde JOIN public.programs p ON p.current_revision_id=pde.program_revision_id
  WHERE p.id=p_program_id AND pde.removal_mask IS NOT NULL;
  RETURN result;
END $$;

-- Match immutable earliest slots, their position, and recurring day pattern.
-- Missing original lineage yields no targets; shorter prescriptions never substitute another set.
CREATE FUNCTION public.workout_removal_targets(p_program uuid, p_revision uuid, p_day uuid, p_slot uuid, p_order integer)
RETURNS TABLE(id uuid) LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog, public AS $$
  WITH first_revision AS (SELECT r.id FROM public.program_revisions r WHERE r.program_id=p_program ORDER BY r.revision LIMIT 1),
  source AS (SELECT s.*, d.day_index, d.order_in_week FROM public.program_day_exercises s
    JOIN public.program_days d ON d.id=s.program_day_id WHERE s.program_revision_id=(SELECT id FROM first_revision)
    AND d.stable_day_id=p_day AND s.stable_slot_id=p_slot),
  ...
  SELECT target.id FROM public.program_day_exercises target JOIN public.program_days td ON td.id=target.program_day_id
    JOIN public.program_day_exercises original ON original.stable_slot_id=target.stable_slot_id AND original.program_revision_id=(SELECT id FROM first_revision)
    JOIN public.program_days od ON od.id=original.program_day_id CROSS JOIN source CROSS JOIN selected_day
  WHERE td.program_id=p_program AND td.program_revision_id=p_revision
    AND original.exercise_id=source.exercise_id AND original.position=source.position
    AND od.day_index=source.day_index AND od.order_in_week=source.order_in_week
    ...
    AND (p_order IS NULL OR (p_order BETWEEN 1 AND source.set_count AND p_order <= target.set_count
      AND NOT COALESCE(target.removal_mask->'orders','[]'::jsonb) @> to_jsonb(ARRAY[p_order])))
...
$$;
REVOKE ALL ON FUNCTION public.workout_removal_targets(uuid,uuid,uuid,uuid,integer) FROM PUBLIC, anon, authenticated;

CREATE FUNCTION public.preview_workout_removal_v1(p_program_id uuid,p_day_id uuid,p_slot_id uuid,p_order integer DEFAULT NULL)
...
BEGIN
  ...
  IF p.is_active AND p.lifecycle='active' THEN
    SELECT count(*) INTO n FROM public.workout_removal_targets(p.id,p.current_revision_id,p_day_id,p_slot_id,p_order);
  END IF;
  RETURN jsonb_build_object('programId',p.id,'expectedRevision',p.current_revision,'expectedRevisionId',p.current_revision_id,
    'currentStableDayId',p_day_id,'futureCount',n);
END $$;

CREATE FUNCTION public.revise_program_removals_v1(p_payload jsonb) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, extensions AS $$
DECLARE
  uid uuid := auth.uid(); op uuid := (p_payload->>'operationId')::uuid;
  pid uuid := (p_payload->>'programId')::uuid; base uuid := (p_payload->>'expectedRevisionId')::uuid;
  revision integer := (p_payload->>'expectedRevision')::integer; source_day uuid := (p_payload->>'currentStableDayId')::uuid;
  ...
  prior public.program_revision_command_receipts%ROWTYPE; target jsonb; masks jsonb := '{}'::jsonb; mask jsonb; target_id uuid;
  successor uuid := gen_random_uuid(); new_day uuid; receipt jsonb; hash text; n integer; v_snapshot jsonb;
BEGIN
  IF uid IS NULL THEN RAISE EXCEPTION 'unauthenticated'; END IF;
  IF op IS NULL OR pid IS NULL OR base IS NULL OR revision IS NULL OR source_day IS NULL
    OR jsonb_typeof(p_payload->'targets') IS DISTINCT FROM 'array' OR jsonb_array_length(p_payload->'targets')=0 THEN RAISE EXCEPTION 'invalid_input: removal command'; END IF;
  ...
  IF p.current_revision<>revision OR p.current_revision_id<>base THEN RAISE EXCEPTION 'stale_revision: program changed'; END IF;
  ...
  FOR target IN SELECT value FROM jsonb_array_elements(p_payload->'targets') LOOP
    IF NOT EXISTS (SELECT 1 FROM public.program_day_exercises s JOIN public.program_days sd ON sd.id=s.program_day_id
      WHERE sd.program_id=pid AND sd.program_revision_id=base AND sd.stable_day_id=source_day
      AND s.stable_slot_id=(target->>'slotId')::uuid
    ...
    FOR target_id IN SELECT id FROM public.workout_removal_targets(pid,base,source_day,(target->>'slotId')::uuid,(target->>'order')::integer) LOOP
      SELECT COALESCE(masks->target_id::text, removal_mask, '{"version":1,"orders":[],"removed":false}'::jsonb) INTO mask FROM public.program_day_exercises WHERE id=target_id;
      IF target->>'order' IS NULL THEN mask:=mask||'{"removed":true}'::jsonb;
      ELSIF NOT (mask->'orders') @> jsonb_build_array((target->>'order')::integer) THEN
        mask:=jsonb_set(mask,'{orders}',COALESCE(mask->'orders','[]'::jsonb)||jsonb_build_array((target->>'order')::integer));
      END IF;
      masks:=jsonb_set(masks,ARRAY[target_id::text],mask);
    END LOOP;
  END LOOP;
  SELECT count(*) INTO n FROM jsonb_object_keys(masks);
  IF n>0 THEN
    INSERT INTO public.program_revisions SELECT (jsonb_populate_record(NULL::public.program_revisions,to_jsonb(r)||jsonb_build_object(
      'id',successor,'revision',revision+1,'parent_revision_id',base,'source_origin','workout_removal','provenance','workout_removal','snapshot','{}'::jsonb,'payload_hash',''))).*;
    FOR d IN SELECT * FROM public.program_days WHERE program_revision_id=base LOOP
      ...
      FOR ex IN SELECT * FROM public.program_day_exercises WHERE program_day_id=d.id LOOP
        INSERT INTO public.program_day_exercises SELECT (jsonb_populate_record(NULL::public.program_day_exercises,to_jsonb(ex)||jsonb_build_object(
          'id',gen_random_uuid(),'program_day_id',new_day,'program_revision_id',successor,'removal_mask',COALESCE(masks->ex.id::text,ex.removal_mask)))).*;
      END LOOP;
    END LOOP;
    INSERT INTO public.program_generation_context SELECT (jsonb_populate_record(NULL::public.program_generation_context,to_jsonb(c)||jsonb_build_object('id',gen_random_uuid(),'program_revision_id',successor))).*
      FROM public.program_generation_context c WHERE program_revision_id=base;
    SELECT jsonb_build_object('version',1,'kind','workout_removal','parentRevisionId',base,'request',p_payload,'days',jsonb_agg(to_jsonb(pd)||jsonb_build_object(
      'exercises',(SELECT jsonb_agg(to_jsonb(s) ORDER BY position) FROM public.program_day_exercises s WHERE s.program_day_id=pd.id)))) INTO v_snapshot
      FROM public.program_days pd WHERE program_revision_id=successor;
    ...
    UPDATE public.programs SET current_revision=revision+1,current_revision_id=successor,updated_at=now() WHERE id=pid;
  ELSE successor:=base;
  END IF;
  receipt:=jsonb_build_object('operationId',op,'programId',pid,'baseRevisionId',base,'revisionId',successor,
    'revision',revision+CASE WHEN n>0 THEN 1 ELSE 0 END,'changedSlotCount',n,'futureChangedSlotCount',n,'replayed',false);
  INSERT INTO public.program_revision_command_receipts(user_id,operation_id,request_hash,program_id,base_revision_id,successor_revision_id,receipt)
    VALUES(uid,op,hash,pid,base,successor,receipt);
  RETURN receipt;
END $$;

REVOKE ALL ON FUNCTION public.workout_removal_capability_v1(), public.program_removal_state_v1(uuid),
 public.preview_workout_removal_v1(uuid,uuid,uuid,integer), public.revise_program_removals_v1(jsonb) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.workout_removal_capability_v1(), public.program_removal_state_v1(uuid),
 public.preview_workout_removal_v1(uuid,uuid,uuid,integer) TO authenticated;
-- Revision command is internal to atomic workout commands, never a pre-Save side effect.
REVOKE ALL ON FUNCTION public.revise_program_removals_v1(jsonb) FROM authenticated;

CREATE FUNCTION public.validate_workout_removals(p_snapshot jsonb,p_removals jsonb,p_actuals jsonb,p_program jsonb,p_pid uuid,p_day uuid)
...
BEGIN
 ...
 FOR item IN SELECT value FROM jsonb_array_elements(p_removals->'slots') LOOP
   IF NOT EXISTS(SELECT 1 FROM jsonb_array_elements(p_snapshot->'slots') s WHERE s->>'slotId'=item#>>'{}') THEN RAISE EXCEPTION 'invalid_input: removed slot lineage'; END IF;
 END LOOP;
 FOR item IN SELECT value FROM jsonb_array_elements(p_removals->'sets') LOOP
   IF NOT EXISTS(SELECT 1 FROM jsonb_array_elements(p_snapshot->'slots') s,jsonb_array_elements(s->'sets') t
...
BEGIN
  ...
  v_receipt := jsonb_build_object(
    ...
    'correctedAt', now(), 'replayed', false, 'programRemoval',v_program_receipt
  );
...
CREATE FUNCTION public.finalize_workout_removals_v1(p_payload jsonb) RETURNS jsonb
LANGUAGE sql SECURITY INVOKER SET search_path=pg_catalog,public AS $$ SELECT public.finalize_workout_v2(p_payload) $$;
CREATE FUNCTION public.correct_workout_removals_v1(p_payload jsonb) RETURNS jsonb
LANGUAGE sql SECURITY INVOKER SET search_path=pg_catalog,public AS $$ SELECT public.correct_completed_workout_v1(p_payload) $$;
REVOKE ALL ON FUNCTION public.finalize_workout_removals_v1(jsonb),public.correct_workout_removals_v1(jsonb) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.finalize_workout_removals_v1(jsonb),public.correct_workout_removals_v1(jsonb) TO authenticated;
